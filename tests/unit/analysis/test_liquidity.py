...
def _orderbook_from_cents(
    *,
    yes: list[tuple[int, int]] | None,
    no: list[tuple[int, int]] | None,
) -> Orderbook:
    def to_dollars(levels: list[tuple[int, int]] | None) -> list[tuple[str, int]] | None:
        if levels is None:
            return None
        return [(f"{price / 100:.2f}", qty) for price, qty in levels]

    return Orderbook(
        yes_dollars=to_dollars(yes),
        no_dollars=to_dollars(no),
    )


def test_orderbook_depth_rejects_negative_radius() -> None:
    orderbook = _orderbook_from_cents(yes=[(50, 1)], no=[(50, 1)])
...
def test_orderbook_depth_radius_zero_counts_only_exact_mid() -> None:
    ...
    orderbook = _orderbook_from_cents(yes=[(50, 10), (40, 10)], no=[(50, 10), (40, 10)])
...
def test_orderbook_depth_single_level_scores_correctly() -> None:
    orderbook = _orderbook_from_cents(yes=[(50, 100)], no=[(50, 100)])
...
def test_distance_weighting_prefers_near_mid() -> None:
    close = _orderbook_from_cents(yes=[(50, 100)], no=[(50, 100)])
    # midpoint=50; all depth is 10c away
    far = _orderbook_from_cents(
        yes=[(40, 100)],
        no=[(40, 100)],
    )
...
def test_estimate_slippage_small_order_minimal_slippage() -> None:
    orderbook = _orderbook_from_cents(yes=[(47, 1000)], no=[(53, 1000)])
...
def test_estimate_slippage_missing_side_returns_unfillable() -> None:
    ...
    orderbook = _orderbook_from_cents(yes=[(47, 1000)], no=None)
    slip = estimate_slippage(orderbook, "yes", "buy", 10)
...
def test_estimate_slippage_sell_yes_skips_zero_quantity_levels() -> None:
    orderbook = _orderbook_from_cents(yes=[(43, 0), (42, 10)], no=None)
...
def test_estimate_slippage_buy_no_uses_implied_no_asks() -> None:
    orderbook = _orderbook_from_cents(yes=[(60, 10), (55, 10)], no=None)
...
def test_estimate_slippage_sell_no_uses_no_bids() -> None:
    orderbook = _orderbook_from_cents(yes=None, no=[(45, 10), (40, 10)])
...
def test_estimate_slippage_large_order_walks_book() -> None:
    orderbook = _orderbook_from_cents(yes=[(47, 1000)], no=[(53, 100), (52, 100), (51, 100)])
...
def test_estimate_slippage_unfillable_order_reports_remaining() -> None:
    orderbook = _orderbook_from_cents(yes=[(47, 1000)], no=[(53, 50)])
...
def test_liquidity_score_in_range_and_grade(make_market: Any) -> None:
    ...
    orderbook = _orderbook_from_cents(yes=[(47, 500)], no=[(53, 500)])
...
def test_liquidity_score_grades_and_warnings(
    ...
    best_yes = 40
    best_no = 60 - spread
    orderbook = _orderbook_from_cents(yes=[(best_yes, 60)], no=[(best_no, 10)])
...
def test_liquidity_score_warns_on_imbalance(
    ...
    orderbook = _orderbook_from_cents(yes=[(47, yes_qty)], no=[(53, no_qty)])
...
def test_estimate_price_impact_validation_and_cap(make_market: Any) -> None:
    ...
    orderbook = _orderbook_from_cents(yes=[(47, 100)], no=None)  # spread=None -> fallback to 10
