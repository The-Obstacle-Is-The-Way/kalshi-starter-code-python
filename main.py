...
from kalshi_research.clients import Environment, KalshiHttpClient, KalshiWebSocketClient
...
try:
    with Path(KEYFILE).open("rb") as key_file:
        private_key = serialization.load_pem_private_key(
            key_file.read(),
            password=None,  # Provide the password if your key is encrypted
        )
except FileNotFoundError as err:
    raise FileNotFoundError(f"Private key file not found at {KEYFILE}") from err
except Exception as e:
    raise RuntimeError(f"Error loading private key: {e!s}") from e
