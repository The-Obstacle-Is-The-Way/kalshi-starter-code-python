"""HTTP client infrastructure for Exa Websets API."""
...
class ExaWebsetsHttpMixin:
    """
    HTTP client infrastructure for Exa Websets API.

    Provides:
    - Async context manager lifecycle (open/close)
    - Request method with retry logic for transient failures
    ...
    async def __aenter__(self) -> ExaWebsetsHttpMixin:
        await self.open()
        return self
